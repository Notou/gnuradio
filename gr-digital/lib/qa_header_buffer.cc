...
{
    BOOST_CHECK_EQUAL(ex.what(), std::string("header_buffer::extract_field for length must be <= 16"));
    return true;
}
...
BOOST_AUTO_TEST_CASE(test_extract_many)
{
    ...
    BOOST_CHECK_EXCEPTION(header.extract_field<uint16_t>(7, 20, false, true), std::runtime_error, check_error_msg);
